...
一般情况我们都是通过 `动作+业务内容` 进行函数的变量声明,基本命名法还是按照`小驼峰命名`方式,但是，由于我们当下都是 es6 的时代了，前端工程组件里面还是尽可能使用 `const` 来声明函数，如果是组件里面，则必须使用 `const` 声明，因为自动绑定了函数的 `this` 指向,当然也会有小伙伴问为什么不用`let` 去声明，原因也是很简单，因为函数在业务上不应该是一个可被修改的变量，而应该是一个有明确业务目的的 **`常量`** ,这也是为何我们常规意义上应该使用 `const` 去声明函数的原因。
